...
public class CoinJoinClient
{
	...
	private int SafeMiningFeeRate { get; }
	...
	private async Task<RoundState> WaitForBlameRoundAsync(uint256 blameRoundId, CancellationToken token)
	{
		...
		if (roundState.CoinjoinState.Parameters.Network != Network.RegTest && roundState.CoinjoinState.Parameters.MinInputCountByRound < CoinJoinConfiguration.AbsoluteMinInputCount)
		{
			throw new InvalidOperationException($"Blame Round ({roundState.Id}): Abandoning: the minimum input count was too low.");
		}

		if (!roundState.IsBlame && !IsRoundEconomic(roundState.CoinjoinState.Parameters.MiningFeeRate, RoundStatusUpdater.CoinJoinFeeRateMedians, SafeMiningFeeRate, FeeRateMedianTimeFrame))
		{
	...
	public async Task<CoinJoinResult> StartCoinJoinAsync(Func<Task<IEnumerable<SmartCoin>>> coinCandidatesFunc, bool stopWhenAllMixed, CancellationToken cancellationToken)
	{
		...
		do
		{
			...
			{
				if (!IsRoundEconomic(roundParameters.MiningFeeRate, RoundStatusUpdater.CoinJoinFeeRateMedians, SafeMiningFeeRate, FeeRateMedianTimeFrame))
				{
				...
				if (roundParameters.Network != Network.RegTest && roundParameters.MinInputCountByRound < CoinJoinConfiguration.AbsoluteMinInputCount)
				{
			...
			coins = await CoinJoinCoinSelector.SelectCoinsForRoundAsync(coinCandidates, utxoSelectionParameters, liquidityClue).ConfigureAwait(false);
	...
	internal static bool IsRoundEconomic(FeeRate roundFeeRate, Dictionary<TimeSpan, FeeRate> coinJoinFeeRateMedians, int safeMiningFeeRate, TimeSpan feeRateMedianTimeFrame)
	{
		...
		if (safeMiningFeeRate > 1 && roundFeeRate.SatoshiPerByte < safeMiningFeeRate)
		{
			return true;
		}
