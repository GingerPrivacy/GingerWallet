...
public partial class BuyViewModel : RoutableViewModel
{
	...
	public BuyViewModel(UiContext uiContext, IWalletModel wallet)
	{
		...
		NextCommand = ReactiveCommand.CreateFromTask(async () =>
		{
			...
			{
				IsBusy = true;
				var offers = await _wallet.BuyModel.GetOffersAsync(SelectedCurrency.Ticker, amount, SelectedCountry.CountryCode, SelectedCountry.StateCode);
				...
				UiContext.Navigate().To().Offers(_wallet, offers);
			}
			catch (Exception ex)
			{
				Logger.LogError(ex);
		...
		PreviousOrdersCommand = ReactiveCommand.Create(() => UiContext.Navigate().To().Orders(_wallet.BuyModel));
	...
	private async Task<bool> EnsureSelectedCountryAsync()
	{
		...
		_availableCountries = await _wallet.BuyModel.GetCountriesAsync();
	...
	{
		Currencies = await _wallet.BuyModel.GetCurrenciesAsync();
	...
	private async Task SetLimitsAsync()
	{
		try
		{
			...
			var (min, max) = await _wallet.BuyModel.GetLimitsAsync(currency.Ticker, country.CountryCode, country.StateCode);
	...
	protected override void OnNavigatedTo(bool isInHistory, CompositeDisposable disposables)
	{
		...
		this.WhenAnyObservable(x => x._wallet.BuyModel.HasAny)
			.BindTo(this, x => x.HasPreviousOrders)
			.DisposeWith(disposables);

		this.WhenAnyObservable(x => x._wallet.BuyModel.HasHeldOrder)
