	...
	public NewWalletAdvancedOptionsDialogViewModel(CoinJoinProfileViewModelBase currentProfile, bool isAutoCoinjoinEnabled)
	{
		...
		NextCommand = ReactiveCommand.Create(
			() =>
			{
				...
				Close(DialogResultKind.Normal, new Result(new ManualCoinJoinProfileDialogViewModel.ManualCoinJoinProfileDialogViewModelResult(new ManualCoinJoinProfileViewModel(target, safeMiningFeeRate, hours, isolateRed, skipFactors)), IsAutoCoinjoinEnabled));
			});
	}
